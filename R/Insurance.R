...
#'
#' @param insurance_type The insurance type, hence the API that the function will call from. Required. Options are Accident and Health, Car, Endowment, Home and Mortgage, Life, Maternity and Travel. Note: it is recommended to only specify 1 insurance type at a time.
...
ocbc_insurance <- function(insurance_type = c('Accident and Health', 'Car', 'Endowment', 'Home and Mortgage', 'Life', 'Maternity', 'Travel'), acctoken, subCategory = NULL, productName = NULL, benefit = NULL){
  ...
  insurance_function <- function(url_link, acc_token = acctoken , SubCategory = subCategory, ProductName = productName, Benefit = benefit) {
    ...
    if(status$category != "Success") {
      ...
      message("API call for ", paste0(insurance_type, ", "), "successful!")
    }
    if(any(insurance_type == 'Car'| insurance_type == 'Travel')){
      insurance <- content(get_insurance)[[1]][[1]][[2]][[1]][[3]]
      insurance_table <- fromJSON(toJSON(insurance), simplifyDataFrame = TRUE) %>%
        map_dfr(clean_up)
    }
    else if(any(insurance_type == 'Accident and Health'| insurance_type == 'Life'| insurance_type == 'Maternity')){
