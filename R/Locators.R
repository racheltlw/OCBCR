...
#' @param locator_type Locator type, hence the API that the function will call from. Required. Options are "ATM" for ATMs, "Branch" for Branches and Centres, "Deposit" for Deposit Machines and Safe Deposit Boxes. Note: it is recommended to only specify 1 locator type at a time.
...
ocbc_locator <- function(locator_type = c("ATM", "Branch", "Deposit"), acctoken, category = NULL, country = NULL, landmark = NULL, latitude = NULL, longitude = NULL, radius = NULL){
  ...
  locator_function<- function(url_link, acc_token = acctoken, category_ = category, country_ = country, landmark_ = landmark, latitude_ = latitude, longitude_ = longitude, radius_ = radius) {
    ...
    if(locator_type == "Branch"){
      ocbc_locators <- flatten(as.data.frame(fromJSON(toJSON(locator)))) %>%
        mutate_all(~unlist(.)) %>%
        rename("category" = 1, "address" = 2, "landmark" = 3, "latitude" = 4, "longitude" = 5, "postal code" = 6, "opening hours" = 7, "remarks" = 8)
      locations <- st_as_sf(ocbc_locators, coords = c("longitude", "latitude"), crs = 4326)
      map_locator <- mapview(locations)
    }
    else{
      ocbc_locators <- flatten(as.data.frame(fromJSON(toJSON(locator)))) %>%
        mutate_all(~unlist(.)) %>%
        rename("category" = 1, "address" = 2, "landmark" = 3, "latitude" = 4, "longitude" = 5, "postal code" = 6, "remarks" = 7)
      locations <- st_as_sf(ocbc_locators, coords = c("longitude", "latitude"), crs = 4326)
      map_locator <- mapview(locations)
    }
