...
#'
#' @param investment_type The investment type, hence the API that the function will call from. Required. Options are Bonds, Equity and Forex, Listed Securities, Leveraged Forex, Treasury, Unlisted Securities and Unit Trusts. Note: it is recommended to only specify 1 investment type at a time.
...
ocbc_investments <- function(investment_type = c('Bonds', 'Equity and Forex', 'Listed Securities ', 'Leveraged Forex', 'Treasury', 'Unlisted Securities', 'Unit Trusts'), acctoken, productName = NULL, benefit = NULL){
  ...
  if(investment_type == 'Bonds'){
    account_output <- investment_function("https://api.ocbc.com:8243/investments/ospl/bonds/1.0") %>%
      rename("Name of Eligible Trade Account (ETA)" = "eligibleTradeAccounts.name", "ETA description" = "eligibleTradeAccounts.description", "ETA weblink" = "eligibleTradeAccounts.webpageLink" )
  }
