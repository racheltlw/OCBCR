---
output: github_document
---

<!-- README.md is generated from README.Rmd. Please edit that file -->

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/README-",
  out.width = "100%"
)
```
# OCBCR

<!-- badges: start -->
<!-- badges: end -->

The goal of OCBCR is to allow information from a range of OCBC APIs to be accessed easily in an R-readable format. 

## Installation

You can install the released version of OCBCR from [CRAN](https://CRAN.R-project.org) with:

``` r
install.packages("OCBCR")
```

And the development version from [GitHub](https://github.com/) with:

``` r
# install.packages("devtools")
devtools::install_github("QMSS-G5072-2019/Tan_Rachel")
```
## Example

This is a basic example which shows you how to solve a common problem:

```{r example}
library(OCBCR)
ocbc_accounts("Children", acctoken = Sys.getenv("ACC_TOKEN_OCBC"), country = "SG", productName = "Mighty Savers")
```

In that case, don't forget to commit and push the resulting figure files, so they display on GitHub!
